...
const useBooks = () => {
  ...
  const addBook = (book: Book) => {
    const updatedBooks = [...localBooks, { ...book, id: `local-${Date.now()}` }];
    setLocalBooks(updatedBooks);
    localStorage.setItem('localBooks', JSON.stringify(updatedBooks));
    notification.success({ message: 'Success', description: 'Book added successfully' });
  };
  ...
  const editBook = (book: Book) => {
    ...
    setLocalBooks(updatedBooks);
    localStorage.setItem('localBooks', JSON.stringify(updatedBooks));
    notification.success({ message: 'Success', description: 'Book updated successfully' });
  };
  ...
  const deleteBook = (bookId: string | number) => {
    ...
    setLocalBooks(updatedBooks);
    localStorage.setItem('localBooks', JSON.stringify(updatedBooks));
    notification.success({ message: 'Success', description: 'Book deleted successfully' });
  };
